...
/**
 * This class gets the messages for a specific user from the server and displays them in a tableView.
 */
public class messagesController {
	/**
	 * instance variables:
	 * currentUser - the user currently logged in.
	 * res - an arrayList containing all the messages.
	 * msgcontroller - an instance of MsgController used to fetch the messages from the server.
	 */
    @FXML
    ...
    /**
     * loads the user currently logged in into an instance variable, sends a request to the server to get all messages for that user,
     * receives in an arrayList and displays them in a tableView.
     * @param u - the user currently logged in.
     * @param lPC - an instance of LibrarianPageController to update GUI elements in the previous window.
     */
    public void loadUser(User u, LibrarianPageController lPC) {
		...
		for (Message message : res) {
			MessagesWithButton m = new MessagesWithButton(message.getMessageType(),
					message.getTitle(),message.getMsg(),
					message.getBelong(),message.getMessageDate(),message.getUser());
			...
			m.getDelete().setOnAction(e -> {
				//MessagesWithButton selectedItem = (MessagesWithButton) messagesTabel.getSelectionModel().getSelectedItem();
    ...
    /**
     * each row in the tableView contains a button, this method handles the event where that button is pressed.
     * it displays the contents of the message and some actions depending on the message type.
    ...
    private void openViewButton(Message message) throws IOException
    {
    	...
    	else
    	{
    		...
    		primaryStage.initModality(Modality.APPLICATION_MODAL);
    		FXMLLoader loader = new FXMLLoader();
    		Parent root = loader.load(getClass().getResource("/boundary/ViewLockMessage.fxml").openStream());
    		primaryStage.setScene(new Scene(root));
    		viewLockMessageController vLMC = loader.getController();
    		vLMC.loadMessage(message);
    		primaryStage.show();
    	}
    }
