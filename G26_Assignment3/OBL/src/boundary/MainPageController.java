...
/**
 * This is the GUI controller of the main page the user sees when he/she connects to the server.
 * Here the user can search for books or log in to his account.
 */
public class MainPageController {
	/**
	 * instance variables:
	 * userController - an instance of UserController used to send a request to the server to login. 
	 * readerController - an instance of ReaderController used to send a request to the server to search for books.
	 */
	@FXML
	...
	/**
	 * handles the event where the user clicks on "log in" button, sends a request to the server
	 * receives a response from the server and displays a feedback respectively (wrong user ID, wrong password, etc.)
	 * and finally, if the credentials are correct it opens a window depending on the users' role (student, librarian, manager).
	...
	void loginHandler(ActionEvent event) throws IOException {
		...
		if (tg.getSelectedToggle() == (RadioButton) studentRadio) {
			...
			tbl = "librarians";
			fxmlFile = 2;
		} else if (tg.getSelectedToggle() == (RadioButton) managerRadio){
			tbl = "managers";
			fxmlFile = 3;
		} else {
		...
		case 2:
			...
			break;
		case 3:
			root = loader.load(getClass().getResource("/boundary/ManagerPageGUI.fxml").openStream());
			Scene scene11 = new Scene(root);
			scene11.getStylesheets().add(getClass().getResource("buttonDesign.css").toExternalForm());
			primaryStage.setScene(scene11);
			primaryStage.setTitle("Manager Page");
			ManagerPageController mc = loader.getController();
			mc.loadUser(userController.getCurrentUser());
			mc.setNumberOfMessages(userController.getCurrentUser());
			break;
		default:
			wrongInputLabel.setText("No role chosen!");
			wrongInputLabel.setVisible(true);
	...
	/**
	 * handles the event where the user clicks on "Search".
	 * validates that the user did indeed choose a search type and input a keyword, sends a request to the server to search
	 * receives search results from the server and displays in a new window.
	 * @param event - auto-generated by SceneBuilder.
	 */
	@FXML
	...
	void initialize() {
		...
		managerRadio.setToggleGroup(tg);
		...
		Image imgLoginUser = new Image("/pictures/loginLogo.png");
		loginUserImg.setImage(imgLoginUser);
		Image imgSearch = new Image("/pictures/searchBookLogo.png");
		searchImg.setImage(imgSearch);
	}
    /**
     ...
     * it logs the user out, disconnects all clients from the server and exits.
     */
	private void close() {
	...
	/**
	 * this {@code private} method is called upon initialisation, it fills the comboBox with values needed.
	 */
